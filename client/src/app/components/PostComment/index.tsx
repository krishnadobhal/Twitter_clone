...
export default function PostComment(props:any) {
  ...
  return (
    ...
    <div className='m-2 h-screen'>
        {/* Post */}
        { tweet && (
    <div className='border-gray-600 rounded-lg   p-2 sm:p-5  transition-all cursor-pointer bg-slate-900 sm:h-fit'>
      ...
      <div className='col-span-10 sm:col-span-11 '>
        ...
        {/* input */}
        <div className='mt-5'>
     <div className="col-span-10 sm:col-span-11 rounded-xl border mt-2 p-2 bg-black">
        <textarea value={content} 
         onChange={(e) => setContent(e.target.value)}
          placeholder={`Write a comment to ${tweet?.author?.firstName+"'s Post"}`}
          rows={3}
          className=" px-1 bg-transparent border-b border-slate-700 w-full text-lg "
        />
        <div className='flex justify-end'>
        <button className='bg-blue-500 p-2 rounded-md px-10 text-white ml-5 text-xl ' onClick={handleClick}>Reply</button>
        </div>
     </div>
  ...
  {/* {tweet && } */}
   ...
   {tweet && <div className='border border-gray-500 px-1 py-2 mt-2 overflow-y-scroll  no-scrollbar bg-gray-700 rounded-md'>
     { 
     tweet?.comment &&
     tweet?.comment?.map((comment)=>
     
       <div className='border-gray-600  rounded-lg  py-5 p-2 sm:p-5  transition-all cursor-pointer bg-black my-2 border'>
      <div className='grid grid-cols-12  gap-3'>
      <div className='col-span-2 sm:col-span-1'>
        {
